...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet(
				'https://raw.githubusercontent.com/Xyphrion/XyphrionScript/'
					.. readfile('xyphrion/profiles/commit.txt')
					.. '/'
					.. select(1, path:gsub('xyphrion/', '')),
				true
			)
		end)
...
local drawingactor = loadstring(downloadFile('xyphrion/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		local oldreload = shared.xyphrionreload
		vape.Load = function()
		...
		task.spawn(function()
			repeat
				task.wait()
			until not shared.xyphrion
			local executionString = "loadfile('xyphrion/main.lua')(" .. drawingactor .. ')'
			...
			if oldreload then
				executionString = 'shared.xyphrionreload = true\n' .. executionString
			end
...
run(function()
	...
	xyphrion:Clean(Drawing.kill or function() end)
...
run(function()
	...
	entitylib.getEntityColor = function(ent)
		ent = ent.Player
		if not (ent and xyphrion.Categories.Main.Options['Use team color'].Enabled) then
			return
		end
		if isFriend(ent, true) then
			return Color3.fromHSV(
				xyphrion.Categories.Friends.Options['Friends color'].Hue,
				xyphrion.Categories.Friends.Options['Friends color'].Sat,
				xyphrion.Categories.Friends.Options['Friends color'].Value
			)
		end
...
run(function()
    ...
    GrenadeTP = xyphrion.Categories.Blatant:CreateModule({
    	Name = 'GrenadeTP',
    	Function = function(callback)
    		if callback then
    			repeat
...
run(function()
    ...
    GunModifications = xyphrion.Categories.Blatant:CreateModule({
...
run(function()
    ...
    GrenadeESP = xyphrion.Categories.Render:CreateModule({
    	Name = 'GrenadeESP',
    	Function = function(callback)
    		if callback then
...
run(function()
    ...
    NoHurtCam = xyphrion.Categories.Render:CreateModule({
    	Name = 'NoHurtCam',
    	Function = function(callback)
    		if callback then
...
run(function()
    ...
    ThirdPerson = xyphrion.Categories.Render:CreateModule({
    	Name = 'ThirdPerson',
    	Function = function(callback)
    		if callback then
...
run(function()
    ...
    AutoRespawn = xyphrion.Categories.Utility:CreateModule({
    	Name = 'AutoRespawn',
    	Function = function(callback)
    		if callback then
...
run(function()
    ...
    BulletTracers = xyphrion.Categories.Legit:CreateModule({
    	Name = 'BulletTracers',
    	Function = function(callback)
    		if callback then
