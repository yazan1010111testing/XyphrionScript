...
run(function()
    ...
    local function ToolAdded(player, v)
        if v:IsA('Tool') then
            ...
            if Index then
                store[Index] = player
                xyphrion:Clean(v.Destroying:Once(function()
    ...
    local function Added(plr): ...any
        if plr:IsA('Player') then
            xyphrion:Clean(plr.CharacterAdded:Connect(Added))
            xyphrion:Clean(plr:WaitForChild('Backpack', 9e9).ChildAdded:Connect(function(v)
                task.delay(0.2, ToolAdded, plr, v)
            end))
            xyphrion:Clean(plr.ChildAdded:Connect(function(v)
                if v:IsA('Backpack') then
                    xyphrion:Clean(v.ChildAdded:Connect(function(v)
                        task.delay(0.2, ToolAdded, plr, v)
                    end))
                end
            end))
