...
run(function()
	...
	xyphrion:Clean(workspace.BlockContainer.DescendantAdded:Connect(function(v)
		parsePositions(v, function(pos)
			store.blocks[pos] = v
		end)
	end))
	xyphrion:Clean(workspace.BlockContainer.DescendantRemoving:Connect(function(v)
		parsePositions(v, function(pos)
...
run(function()
    ...
    NoFall = xyphrion.Categories.Blatant:CreateModule({
        Name = 'No Fall',
        Function = function(callback)
            if callback then
                repeat
...
run(function()
    ...
    local ProjectileAimbot = xyphrion.Categories.Blatant:CreateModule({
    ...
    Viewmodel = vape.Legit:CreateModule({
        ...
        Function = function(callback)
            if callback then 
                ...
                xyphrion:Clean(ViewmodelMotor)
                xyphrion:Clean(runService.RenderStepped:Connect(function()
                ...
                xyphrion:Clean(entitylib.Events.LocalAdded:Connect(newCharacter))
                if entitylib.isAlive then 
