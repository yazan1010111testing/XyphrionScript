...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/Xyphrion/XyphrionScript/'..readfile('xyphrion/profiles/commit.txt')..'/'..select(1, path:gsub('xyphrion/', '')), true) end)
...
local vm = loadstring(downloadFile('xyphrion/libraries/vm.lua'), 'vm')()
...
run(function()
	...
	if not jb.VehicleController.toggleLocalLocked or not jb.VehicleController.NitroShopVisible then
		repeat task.wait() until (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) or xyphrion.Loaded == nil
...
run(function()
    ...
    AutoPop = xyphrion.Categories.Blatant:CreateModule({
    	Name = 'AutoPop',
    	Function = function(callback)
    		if callback then
    			task.spawn(function()
...
run(function()
    ...
    AutoPunch = xyphrion.Categories.Blatant:CreateModule({
    	Name = 'AutoPunch',
    	Function = function(callback)
    		if callback then
    			repeat
...
run(function()
    ...
    AutoTaze = xyphrion.Categories.Blatant:CreateModule({
    	Name = 'AutoTaze',
    	Function = function(callback)
    		if callback then
    			repeat
...
run(function()
    LazerGodmode = xyphrion.Categories.Blatant:CreateModule({ Name = 'LazerGodmode' })
end)
...
run(function()
    ...
    InfNitro = xyphrion.Categories.Utility:CreateModule({
    	Name = 'InfiniteNitro',
    	Function = function(callback)
    		if callback then
